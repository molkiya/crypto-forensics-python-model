...
class ChebyshevConvolution(Module):
    def __init__(self, args, kernel, num_features, hidden_units):
        ...
        # Третий сверточный слой: продолжает обработку скрытых признаков
        self.conv3 = ChebConv(hidden_units, hidden_units, kernel[2])
        # Четвертый (выходной) слой: преобразует признаки в количество классов
        self.conv4 = ChebConv(hidden_units, args['num_classes'], kernel[3])
    ...
    def forward(self, data):
        ...
        # Третий сверточный слой с ReLU6
        x = F.relu6(self.conv3(x, edge_index))
        # Dropout для регуляризации
        x = F.dropout(x, p=0.5, training=self.training)

        # Четвертый (выходной) слой, без функции активации
